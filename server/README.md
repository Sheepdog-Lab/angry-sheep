...
OpenCV can briefly “see” wrong marker IDs (e.g. 17, 37) on glare or clutter. **`ALLOWED_MARKER_IDS`** in `server.py` is **`None` by default** (all IDs 0–49 for DICT_4X4_50). Set it to `frozenset({...})` to restrict output. Match **`allowedMarkerIds`** in `web/src/config.js` for the overlay. Physical **block** markers are mapped in **`web/src/physicalMode.js`** (IDs **7–20**).
